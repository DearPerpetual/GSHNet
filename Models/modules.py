...
class GCA(nn.Module):
    def __init__(self, dim1, dim2, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.rel_pos_h = nn.Parameter(torch.randn(num_heads, 1, 64))
        self.rel_pos_w = nn.Parameter(torch.randn(num_heads, 1, 64))

        self.spatial_conv = nn.Sequential(
            nn.Conv1d(dim1, dim1, kernel_size=3, padding=1, groups=dim1),
            nn.BatchNorm1d(dim1),
    ...
    def forward(self, fea, depth_fea):
        B, N1, _ = fea.shape
        _, N2, _ = depth_fea.shape
        C = self.dim

        conv_fea = self.spatial_conv(fea.transpose(1, 2)).transpose(1, 2)
        q1 = self.q1(conv_fea).reshape(B, N1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k2 = self.k2(depth_fea).reshape(B, N2, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v2 = self.v2(depth_fea).reshape(B, N2, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        attn = (q1 @ k2.transpose(-2, -1)) * self.scale

        if N1 <= 64 and N2 <= 64:
            attn = attn + self.rel_pos_h[:, :, :N1] + self.rel_pos_w[:, :, :N2]

        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        out = (attn @ v2).transpose(1, 2).reshape(B, N1, C)
        out = self.proj(out)
...
class LFSG(nn.Module):
    def __init__(self, dim, img_size, window_size, num_heads=1, mlp_ratio=3, drop_path=0.):
        super(LFSG, self).__init__()

        self.img_size = img_size  # (H, W)
        self.dim = dim
        self.mlp_ratio = mlp_ratio

        self.windowatt = WindowAttentionBlock(
            dim=dim, input_resolution=img_size, num_heads=num_heads, window_size=window_size, shift_size=0,
        ...
        self.globalatt = Block(
            dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=False, qk_scale=None, drop=0.,
        ...
        self.mlp = ConvFFN(dim, mlp_ratio)
    ...
    def forward(self, x):
        ...
        att_fused = self.fusion(att1 + att2)
        x = x + att_fused
