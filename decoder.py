...
from Models.modules import LFSG
...
class decoder(nn.Module):
    ...
    def __init__(self,embed_dim=384,dims=[96,192,384],img_size=224,mlp_ratio=3):
        ...
        self.fusion1 = MLFF(in_dim=dims[2],f_dim=dims[1],kernel_size=(3,3),img_size=(img_size//8,img_size//8),stride=(2,2),padding=(1,1))
        self.fusion2 = MLFF(in_dim=dims[1],f_dim=dims[0],kernel_size=(3,3),img_size=(img_size//4,img_size//4),stride=(2,2),padding=(1,1))
        self.fusion3 = MLFF(in_dim=dims[0],f_dim=dims[0],kernel_size=(7,7),img_size=(img_size//1,img_size//1),stride=(4,4),padding=(2,2),fuse=False)

        self.mixatt1 = LFSG_block(in_dim=dims[1],dim=embed_dim,img_size=(img_size//8,img_size//8),window_size=(img_size//32),num_heads=1,mlp_ratio=mlp_ratio,depth=2)
        self.mixatt2 = LFSG_block(in_dim=dims[0],dim=embed_dim,img_size=(img_size//4,img_size//4),window_size=(img_size//32),num_heads=1,mlp_ratio=mlp_ratio,depth=2)
    ...
    def forward(self,f):
        fea_1_16,fea_1_8,fea_1_4 = f #fea_1_16:1/16   this is 16,8,4 torch.Size([2, 576, 256]) torch.Size([2, 2304, 128]) torch.Size([2, 9216, 64])
        B,_,_ = fea_1_16.shape

        fea_1_8 = self.fusion1(fea_1_16,fea_1_8) +fea_1_8  #  fea_1_8:torch.Size([2, 2304, 128]) out of MLFF
        fea_1_8 = self.mixatt1(fea_1_8)   #fea_1_8:torch.Size([2, 2304, 128])  out of MAB

        fea_1_4 = self.fusion2(fea_1_8,fea_1_4)  #out of MLFF
        fea_1_4 = self.mixatt2(fea_1_4)  # out of MAB  #this is 4 torch.Size([2, 9216, 64])
        fea_1_1 = self.fusion3(fea_1_4)   #this is 1 torch.Size([2, 147456, 64])
...
class MLFF(nn.Module):
    ...
    def forward(self,fea,fea_1=None):
        ...
        B, C, _, _ = fea.shape
        fea = fea.view(B, C, -1).transpose(1, 2)
        if self.fuse:
...
class LFSG_block(nn.Module):
    def __init__(self,in_dim,dim,img_size,window_size,num_heads=1,mlp_ratio=4,depth=2,drop_path = 0.):
        ...
        self.blocks = nn.ModuleList([
            LFSG(dim=dim,img_size=img_size,window_size = window_size,num_heads=num_heads,mlp_ratio=mlp_ratio)
